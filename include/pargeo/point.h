...
namespace pargeo {
    ...
    public:
        ...
        template<class pt>
        // todo add template for all (for derived class)
        pt operator-(pt op2) {
            _tData xx[_dim];
            for (int i = 0; i < _dim; ++i) xx[i] = x[i] - op2.x[i];
            return pt(xx, attribute);
    ...
    template<int dim>
    using point = _point<dim, double, double, double *>;

    template<int dim>
    using wpoint = _point<dim, double, double, double *>;

    template<int dim>
    using fpoint = _point<dim, float, float, double *>;

    template<int dim>
    using lpoint = _point<dim, long, double, double *>;
